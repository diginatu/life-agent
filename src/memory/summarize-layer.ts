...
export async function summarizeL3(
  ...
  const lines = l2Entries
    ...
    .join("\n");
  const prompt = `Summarize the following hourly summaries for time window "${windowLabel}". Write 2-3 concise sentences describing what the person was doing, their posture, and any notable events, actions or user information.\n\nHourly summaries:\n${lines}`;
