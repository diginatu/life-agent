...
export const DEFAULT_L4_PROMPT = `Update the persistent memory with any new durable facts, user preferences, user patterns, habit, or events from the expiring 6-hour summary. Prioritize the user requests. Preserve existing important facts; drop stale or trivial details. Keep the result concise (under {l4MaxChars} characters). Output only the new persistent memory text, no preamble.
