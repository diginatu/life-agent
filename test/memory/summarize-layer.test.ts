...
import { summarizeL2 } from "../../src/memory/summarize-layer.ts";
...
describe("summarizeLayer", () => {
  test("returns LLM output string", async () => {
    ...
    const result = await summarizeL2(ollama, sampleEntries, "2026-04-15T10");
  ...
  test("prompt contains window label", async () => {
    ...
    await summarizeL2(ollama, sampleEntries, "2026-04-15T10");
